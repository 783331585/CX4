    ...
    public static class LinqExtensions
    {
        ...
        public static Expression<Func<T, bool>> GetAndLambdaExpression<T>(List<SearchModel> queryList)
        {
            ...
            foreach (SearchModel item in queryList)
            {
                ...
                Expression filter = item.Condition switch
                {
                    //em查询条件类型.等于:
                    ...
                    // em查询条件类型.大于:
                    "More" => Expression.GreaterThan(left, right),
                    //查询条件类型.大于等于:
                    "MoreEqual" => Expression.GreaterThanOrEqual(left, right),
                    // em查询条件类型.小于:
                    "Less" => Expression.LessThan(left, right),
                    // em查询条件类型.小于等于:
                    "LessEqual" => Expression.LessThanOrEqual(left, right),
                    //em查询条件类型.like:
                    "Like" => Expression.Call(left, typeof(string).GetMethod("Contains"), right),
                    // em查询条件类型.notlike:
                    "NotLike" => Expression.Not(Expression.Call(left, typeof(string).GetMethod("Contains"), right)),
